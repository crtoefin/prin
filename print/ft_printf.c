...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/24 14:37:36 by crtoefin          #+#    #+#             */
/*   Updated: 2021/12/24 17:21:39 by                  ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_ui(unsigned int un, int *ptr_j, char c)
{
	if (c == 'u')
...
int	ft_printf(const char *c, ...)
{
	...
	while (c[s.i])
	{
		...
		|| c[s.i + 1] == 'd'))
			s.i += ft_int(va_arg(ptr, int), s.ptr_j, c[s.i + 1]);
		else if (c[s.i] == '%' && c[s.i + 1] == '%')
			s.i += (ft_putchar('%', s.ptr_j) + 1);
		else if (c[s.i] == '%' && (c[s.i + 1] == 's' || c[s.i + 1] == 'p'))
			s.i += ft_void(va_arg(ptr, void *), s.ptr_j, c[s.i + 1]);
		else if (c[s.i] == '%' && (c[s.i + 1] == 'u' || c[s.i + 1] == 'x' \
		|| c[s.i + 1] == 'X'))
			s.i += ft_ui(va_arg(ptr, unsigned int), s.ptr_j, c[s.i + 1]);
		else
